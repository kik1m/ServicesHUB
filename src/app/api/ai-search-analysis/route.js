...
export async function POST(req) {
    // Initialize Supabase Client inside handler (safe for build time)
    ...
    try {
        ...
        if (userId) {
            ...
            if (profile) {
                ...
                if (!isPremium) {
                    ...
                    if (lastSearch && (now.getTime() - lastSearch.getTime() < 86400000) && count >= 3) {
                        return NextResponse.json({ error: 'Limit Reached', message: 'Daily limit reached.' }, { status: 429 });
                    }
                }
            }
        }
        ...
        if (userId && !isPremium) {
            ...
            if (profileData?.last_ai_search_at && (now.getTime() - new Date(profileData.last_ai_search_at).getTime() > 86400000)) newCount = 1;
