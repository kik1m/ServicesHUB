...
const BASE_DETAIL_SELECT = `
    ...
    url, pricing_type, pricing_details, pricing_details_full, pricing_url, rating, reviews_count, 
...
export const toolsService = {
    ...
    async getToolsPaginated({
        page = 0,
        offset = null, // New: Direct offset support
        ...
        // 📏 5. Pagination
        const from = offset !== null ? offset : (page * itemsPerPage);
        const to = from + itemsPerPage - 1;

        // Rule #11.2: Defensive Pagination Check
        // If we are requesting a range that is clearly beyond what we've seen,
        // we should handle it gracefully to avoid 416 errors.
        query = query.range(from, to);

        const response = await query;
        
        // Handle 416 error gracefully (Requested Range Not Satisfiable)
        // Rule #11.2: If we hit the end, return empty data but keep the last known state
        if (response.error && response.error.code === 'PGRST103') {
            return { data: [], count: null, error: null }; // Returning null count prevents overwriting the total
        }
