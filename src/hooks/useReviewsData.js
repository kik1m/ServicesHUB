...
/**
 * 🚀 Elite Custom hook for managing tool review lifecycle (React Query)
...
export const useReviewsData = (toolId, labels, onReviewSuccess) => {
    ...
    const submitReply = async (parentReviewId, isOwner = false) => {
        ...
        try {
            ...
            queryClient.setQueryData(['reviews', toolId], prev => [data, ...(prev || [])]);
            setReplyComment('');
            setReplyingTo(null);
    ...
    const handleDeleteReview = async (reviewId) => {
        ...
        try {
            ...
            queryClient.setQueryData(['reviews', toolId], prev => (prev || []).filter(r => r.id !== reviewId));
