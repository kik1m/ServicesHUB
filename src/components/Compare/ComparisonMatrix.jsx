...
const ComparisonMatrix = ({ tool1, tool2, isLoading, isTool1Loading, isTool2Loading, isAiLoading, aiResults, aiError, error, onRetry, content }) => {
    ...
    // --- 🚀 ELITE UI TEXT FORMATTER (Rule #10 Compliance) ---
    const renderStructuredText = (text) => {
        ...
        // Sanitize: Remove stray pipes
        let cleanText = textStr.replace(/\|/g, '').trim();

        // Split by our new deterministic separator [SPLIT] 
        // OR fallback to old separators (• or .,) for cached data
        let items = cleanText.split(/\[SPLIT\]|•|(\.,)/).map(t => t?.trim()).filter(t => t && t !== '.,');
        
        if (items.length <= 1) {
            // Last resort: split by sentences only if absolutely no other markers found
            const alternativeItems = cleanText.split(/\. (?=[A-Z])/);
            if (alternativeItems.length > 1) items = alternativeItems;
            else return <p>{cleanText}</p>;
        }

        return (
            <ul className={styles.structuredList}>
                {items.map((item, idx) => {
                    // Cleanup trailing punctuation that might look messy in a list
                    const sanitizedItem = item.replace(/[.,]$/, '').trim();
                    
                    // Handle **Bold** text within the items
                    const parts = sanitizedItem.split(/(\*\*.*?\*\*)/);
                    return (
                        <li key={idx} className={styles.structuredItem}>
                            {parts.map((part, pIdx) => {
                            ...
                            {isAiLoading ? (
                                ...
                                <div className={styles.aiInsights}>
                                    <div className={styles.strategicGrid}>
                                        {String(aiResults.strategic_overview).split('[SPLIT]').map((text, idx) => (
                                            <div key={idx} className={styles.strategicCard}>
                                                <div className={styles.strategicCardHeader}>
                                                    <Sparkles size={14} />
                                                    <span>{idx === 0 ? tool1.name : tool2.name} Positioning</span>
                                                </div>
                                                <p className={styles.strategicText}>{text.trim()}</p>
                                            </div>
                                        ))}
                                    </div>
