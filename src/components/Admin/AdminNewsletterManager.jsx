...
/**
 * AdminNewsletterManager - Elite Campaign Orchestrator (Next.js Port)
 */
const AdminNewsletterManager = ({ 
    ...
    if (activeTab !== 'newsletter-manager') return null;
    ...
    useEffect(() => {
        const fetchTools = async () => {
            ...
            try {
                let query = supabase
                    .from('tools')
                    .select('id, name, image_url, short_description, slug')
                    .eq('is_approved', true)
            ...
            <div className={styles.grid}>
                <div className={styles.configPanel}>
                    <section className={styles.section}>
                        ...
                        <div className={styles.inputGroup}>
                            <label className={styles.label}>Intro Message (Editor's Note)</label>
                            <textarea 
                    ...
                    <section className={styles.section}>
                        ...
                        <div className={styles.toolSelector}>
                            {isSearching ? (
                                <div className={styles.searchingMsg}>Searching HUBly database...</div>
                            ) : liveTools.map(tool => {
                                ...
                                return (
                                    ...
                                    >
                                        <SmartImage src={tool.image_url} alt={tool.name} containerClassName={styles.toolIcon} />
                                        <span className={styles.toolName}>{tool.name}</span>
                                        {isSelected ? <Trash2 size={16} className={styles.removeIcon} /> : <Plus size={16} />}
                                    </div>
                                );
                            })}
                        </div>
                                ...
                                {campaignData.selectedTools.length === 0 ? (
                                    ...
                                    campaignData.selectedTools.map(tool => (
                                        <div key={tool.id} className={styles.previewToolCard}>
                                            <SmartImage src={tool.image_url} alt="" containerClassName={styles.previewToolImg} />
