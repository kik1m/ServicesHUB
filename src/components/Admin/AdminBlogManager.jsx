...
import { Trash2, PenTool, BookOpen, Eye, Plus, Search, Image as ImageIcon, Type, GitCompare } from 'lucide-react';
...
/**
 * AdminBlogManager - Elite Modular Blog Editor (Next.js Port)
 */
const AdminBlogManager = memo(({ 
    ...
    if (activeTab !== 'blog' || !isMounted) return null;
    ...
    return (
        <Safeguard error={error} onRetry={onRetry}>
            {actionError && <div className={styles.localError}>{actionError}</div>}
            <div className={styles.splitLayout}>
                <div className={styles.formCol}>
                        ...
                        <div className={styles.toolPicker}>
                            ...
                            <div className={styles.toolResults}>
                                {displayTools.length > 0 ? displayTools.map(tool => (
                                    ...
                                    <p className={styles.noResults}>No tools found.</p>
                                )}
                            </div>
                        </div>
                    )}
                        ...
                        {!isPreview ? (
                            ...
                            <div className={styles.previewContainer}>
                                ...
                                <div className={styles.previewMeta}>
                                    {blogCategories.find(c => c.id === newPost.category || c.name === newPost.category)?.name || 'No Category'} • Live Preview
                                </div>
                                <div 
                                    ...
                                    dangerouslySetInnerHTML={{ 
                                        __html: (newPost?.content || '')
                                            .replace(/\[tool id="([^"]+)"\]/g, (match, id) => {
                                                ...
                                                return `<div style="padding:25px; background:linear-gradient(135deg, rgba(0,210,255,0.1) 0%, rgba(0,210,255,0.05) 100%); border:1px solid #00a3ff; border-radius:20px; margin:30px 0; display:flex; justify-content:space-between; align-items:center;">
                                                    <div style="display:flex; flex-direction:column; gap:5px;">
                                                        <span style="font-size:0.8rem; color:#00a3ff; font-weight:800; text-transform:uppercase; letter-spacing:1px;">Featured Tool</span>
                                                        <span style="font-size:1.3rem; font-weight:800; color:white;">${toolName}</span>
                                                    </div>
                                                    <div style="background:#00a3ff; padding:10px 20px; border-radius:12px; font-weight:bold; color:white; font-size:0.9rem;">View in Article</div>
                                                </div>`;
                                            })
                                    }} 
                                />
