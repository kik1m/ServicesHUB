 ...
 */
async function scrapeUrlToMarkdown(url, pricingUrl = null) {
    let exactImageUrl = null;
    let markdown = null;

    try {
        console.log(`🔍 Scraping URL: ${url}`);
        if (pricingUrl) console.log(`💰 Explicit Pricing URL detected: ${pricingUrl}`);
        
        // 1. AGGRESSIVE IMAGE EXTRACTION & LINK DISCOVERY (Raw HTML)
        let subPagesToScrape = [];
        if (pricingUrl) subPagesToScrape.push(pricingUrl);
            ...
            try {
                ...
                const response = await axios.get(jinaUrl, {
                    headers: {
                        ...
                        'X-Wait-For-Selector': '.pricing, [class*="pricing"], #pricing, table', // Wait for common pricing selectors
                        'X-Return-Format': 'markdown'
                    },
                    timeout: 30000 // Increased timeout for heavy JS pages
                });
